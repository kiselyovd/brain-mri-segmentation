...
evaluate:
	uv run python -m brain_mri_segmentation.evaluation.evaluate --checkpoint artifacts/checkpoints/best.ckpt --data data/processed

serve:
	MODEL_PATH=artifacts/checkpoints/best.ckpt uv run uvicorn brain_mri_segmentation.serving.main:app --reload --port 8000
test:
	uv run pytest
